...
from m16e.kommon import KQR_SELECTED_ID, ACT_CLEAR, KDT_INT, KDT_CHAR, storagize
...
class AttachTypeIndexView( BaseListPlasticView ):
    controller_name = 'attach_type'
    function_name = 'index'


    def __init__( self, db ):
        super( AttachTypeIndexView, self ).__init__( db )
    ...
    def get_table_view_dict( self ):
        T = current.T
        tdef = { KTF_COL_ORDER: [ 'id', 'name', 'meta_name' ],
                 KTF_SORTABLE_COLS: [ 'id', 'name', 'meta_name' ],
                 KTF_CELL_CLASS: 'table_border',
                 KTF_COLS: { 'id': { KTF_TITLE: T( 'Attach type Id' ),
                                     KTF_TYPE: KDT_INT,
...
#
#         if action == ACT_NEW_ATTACH_TYPE:
#             redirect = URL( c='attach_type', f='edit', args=[ 0 ] )
#
#         qd = getQueryData()
#
...
#         main_panel = DIV()
#         main_panel.append( H3( T( 'Attach type list' ) ) )
#         table = TABLE()
#         tr = TR()
#         tr.append( TH( T( 'Id' ) ) )
#         tr.append( TH( T( 'Path' ) ) )
...
#                            'next:f': next_f,
#                            'next_args': next_args }
#             url = URL( c='attach_type', f='edit', args=[ r.id ], vars=d_vars )
