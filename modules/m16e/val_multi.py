...
def _valN(num):
    ...
    chkDigit = (sum % 11 and (11 - sum % 11) % 10)
#    term.printLog( 'chkDigit: %s' % ( repr( chkDigit ) ) )
    # verificar soma de controlo
    return chkDigit == num[-1]
...
#    term.printDebug( 'nif: %s' % repr( nif ) )
    # remove spaces
    nif = nif.replace( ' ', '' )
    ...
    if len(nif) != LEN_NIF:
        term.printLog( 'bad len: %d' % ( len( nif ) ) )
        return False

    # verificar validade do caractere inicial do NIF
    if nif[0] not in "12568":
        v = False
        a2 = [ '45',
        ...
        for a in a2:
            if nif.startswith( a ):
                ...
                break
        if not v:
            term.printLog( 'bad start digit: %s' % ( nif[0] ) )
            return False
...
if __name__ == "__main__":
    ...
    print "Testar contribuinte 123456789:", \
                controlNIF("123456789")
    print "Testar contribuinte 505213737:", \
                controlNIF("505213737")
    print "Testar contribuinte 505213738:", \
                controlNIF("505213738")
