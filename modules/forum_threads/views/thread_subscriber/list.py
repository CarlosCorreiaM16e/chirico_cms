...
KQV_USER_ID = 'KQV_PREFIX + user_id' 
KQV_REQUEST_ID = 'KQV_PREFIX + request_id' 

#----------------------------------------------------------------------
class RequestSubscribersListView( BaseListPlasticView ):
    controller_name = 'request_subscriberss'
    function_name = 'list'

    #----------------------------------------------------------------------
    def __init__( self, db ):
        super( RequestSubscribersListView, self ).__init__( db )

        T = current.T
        self.table_model = db_tables.get_table_model( 'request_subscribers', db=db )

        self.list_title = T( 'RequestSubscribers list' )

        self.append_var( KQV_USER_ID, fld_type=KDT_INT )
...
#----------------------------------------------------------------------
    def get_table_view_dict( self ):
        T = current.T
        tdef = { KTF_COL_ORDER: [ 'id', 'request_id', 'user_id' ],
                 KTF_SORTABLE_COLS: [ 'id', 'request_id', 'user_id' ],
...
#----------------------------------------------------------------------
    def get_table_qdata_dict( self, extra_buttons={} ):
        ...
        qdata = { KTF_BUTTONS: [],
                  KTF_COL_ORDER: [ KQV_USER_ID, KQV_REQUEST_ID, KQV_SHOW_ALL ],
...
#----------------------------------------------------------------------
    def get_query_data( self, orderby=None ):
        qd = super( RequestSubscribersListView, self ).get_query_data( orderby )
        qv_user_id =  self.query_vars.get( KQV_USER_ID )
        if qv_user_id:
            qd.addAnd( QueryData( 'user_id = %(qv_user_id)s',
